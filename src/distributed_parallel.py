...
from datasets.luna import Luna16
import mytransforms
...
class ReversibleDistributedDataParallel(nn.DistributedDataParallel):
    ...
    def backward(self, *inputs, **kwargs):
        if self.device_ids:
            ...
            if len(self.device_ids) == 1:
                output = self.module.backward(*inputs[0], **kwargs[0])
            else:
                outputs = self.parallel_apply_backward(self._module_copies[:len(inputs)], inputs, kwargs)
